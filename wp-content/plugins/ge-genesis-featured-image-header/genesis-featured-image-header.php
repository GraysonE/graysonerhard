<?php
/*
Plugin Name: Genesis Featured Image Header - DO NOT DELETE (EDITED)!
Plugin URI: https://surpriseazwebservices.com/plugins/genesis-featured-image-header/
Description:  DO NOT DELETE (EDITED)! Easily add a featured image to the header of every page on your site including Custom Post Type archive pages.
Version: 1.2.6
	...
	foreach ( $post_types  as $post_type ) {
		if(is_post_type_archive($post_type) && !is_admin()) {
			if(get_option('gfih_' . $post_type . '_featured_image')){
				echo '<div class="featured_image_wrapper"><img src="' . get_option('gfih_' . $post_type . '_featured_image') . '" class="attachment-full wp-post-image" alt="' . $post_type . '" /></div>';
			}
		}
	}
}
